...
DEFAULT_PIPECONF = "p4adhocproject.pipeconf"
...
def watchDog(sw):
    try:
        ...
        while True:
            if RustyBmv2Switch.mininet_exception == 1 \
...
class RustyBmv2Switch(Switch):
    ...
    def getDeviceConfig(self, srcIP):
        ...
        basicCfg = {
            ...
            "socket_addr": "%s:%s"%(srcIP,self.grpcPort),
            "device_id": self.p4DeviceId
        }
        ...
        if not self.useStratum and self.injectPorts:
            ...
            for intf in self.intfList():
                portName = "of:%s/%s" % (self.onosDeviceId, str(portId))
                intfName = str(intf)
                if intfName == 'lo':
                    continue
                portData[portName] = {
        ...
        portId = 1
        for intf in self.intfList():
            portName = "%s/%s" % (self.onosDeviceId, str(portId))
            intfName = str(intf)
            if intfName== 'lo':
                continue
            if hasattr(intf,'mac'):
    ...
    def chassisConfig(self):
        ...
        for intfName in self.intfNames():
            ...
            config = config + """singleton_ports {{
                ...
                node: {nodeId}
            }}\n""".format(intfName=intfName, intfNumber=intfNumber, nodeId=self.p4DeviceId)
            intfNumber += 1
    ...
    def start(self, controllers):
        ...
        try:
            ...
            RustyBmv2Switch.mininet_exception = 1
            self.killBmv2()
            self.printBmv2Log()
            raise
