...
impl P4InfoHelper {
    ...
    pub fn build_table_entry(
        ...
        for (match_field_name, value) in match_fields {
            let entry = self
                .get_match_field_pb(table_name, match_field_name, value)
                .unwrap();
        ...
        if !action_name.is_empty() {
            ...
            if !action_params.is_empty() {
                for (field_name, value) in action_params {
                    action
                        .params
                        .push(self.get_action_param_pb(action_name, field_name, value));
                }
            }
        }
        return table_entry;
    ...
    pub fn get_match_field_pb(
        ...
        let p4info_match = self
            .get_match_field_by_name(table_name, match_field_name)
            .unwrap();
    ...
    pub fn get_action_param_by_name(
        &self,
        action_name: &str,
        param: &str,
    ) -> Option<&Action_Param> {
    ...
    pub fn get_action_param_pb(
        ...
        let p4info_param = self
            .get_action_param_by_name(action_name, param_name)
            .unwrap();
