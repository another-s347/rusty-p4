...
pub fn main() {
    ...
    s1.master_arbitration_update().unwrap();
    s2.master_arbitration_update().unwrap();
...
fn write_tunnel_rules(
    ...
    ingress_sw.write_table_entry(table_entry).unwrap();
    ...
    .to_table_entry(&pipeconf, 0);
    egress_sw.write_table_entry(table_entry).unwrap();
}
