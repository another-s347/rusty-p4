...
use super::{request, server::Server, DefaultRequest, ParseRequest, RequestOption, Service};
...
impl ParseRequest for DummyRequest {
    fn parse(req: super::DefaultRequest) -> crate::error::Result<Self> {
        Ok(DummyRequest { action: req.action })
    }
}
...
impl super::Service for DummyService {
    ...
    fn process(
        &mut self,
        request: super::Request<Self::Request>,
        ...
        match request.inner.action.as_ref() {
            ...
            other => Err(crate::error::ServiceError::ActionNotFound(other.to_owned()).into()),
        }
    }
}
...
impl DummyServer {
    pub async fn run(
        &self,
        service_bus: &super::ServiceBus,
        request: DefaultRequest,
    ) -> crate::error::Result<Vec<String>> {
        let mut response = service_bus
            .send::<Self>(DummyService::NAME, request, RequestOption::default())
            .await?;
